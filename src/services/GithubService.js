/* globals PullRequestTransformer, get, getJSON, GithubAPI */
...
function GithubService() {
    ...
    this.getPullRequests = function (owner, repo, callback) {
        getJSON(api.urlBuilder('repos/' + owner + '/' + repo + '/pulls?state=all', {}), function (pullrequests) {
    ...
    this.getPullRequest = function (owner, repo, number, callback) {
        getJSON(api.urlBuilder('repos/' + owner + '/' + repo + '/pulls' + '/' + number, {}), function (pullrequest) {
            var transformer, transformed;
            transformer = new PullRequestTransformer();
    ...
    function getFilesChanged(owner, repo, number, callback) {
        getJSON(api.urlBuilder('repos/' + owner + '/' + repo + '/pulls' + '/' + number + '/files', {}), function (files) {
            var transformer, transformed;
            transformer = new PullRequestTransformer();
