var modules = []; //Global modules list for if the module objects are needed elsewhere.

/**	svg line creator
* data - The data in JSON with defined x and y fields
* interpolationType - String describing the path shape, defaults to cardinal. See https://github.com/d3/d3/wiki/SVG-Shapes#line_interpolate for supported types.
...
function line(data, interpolationType = "cardinal", xFunction = null, yFunction = null) {
	var lineFunction = d3.svg.line()
		.x(function(d) { if(xFunction===null) return d.x; else return xFunction(d.x); })
		.y(function(d) { if(yFunction===null) return d.y; else return yFunction(d.y); })
...
function area(data, height, interpolationType = "cardinal", xFunction = null, yFunction = null) {
	var areaFunction = d3.svg.area()
		.x(function(d) { if(xFunction===null) return d.x; else return xFunction(d.x); })
    	.y0(height)
		.y1(function(d) { if(yFunction===null) return d.y; else return yFunction(d.y); })
		.interpolate(interpolationType);
